...
#
# def loadYamls(pathnames):
#     if type(pathnames) is str:
#         pathnames = [pathnames]
#     yamls = []
#     for file in pathnames:
#         with open(file, 'r') as ymlfile:
#             yamls.append(yaml.load(ymlfile, Loader=loader))
#         # print('Loaded Yaml from %s' % pathnames)
#     if len(yamls) == 1:
#         yamls = yamls[0]
#     return yamls
#
# def saveYamls(pathname, dict):
#     with open(pathname, 'w') as ymlfile:
#         yaml.dump(dict, ymlfile, Dumper=dumper)
#     print('Saved Yaml to %s' % pathname)
#
#
# def expandFolders(directory):
#     if type(directory) is str:
#         return directory
#
#     folders = []
...
#
#
# def getFilenames(dictionary):
#     returnDict = dictionary.copy()
#     folders = expandFolders(dictionary['dir'])
#     if type(folders) is str:
#         folders = [folders]
#     listOfFiles = []
#     for fldr in folders:
...
#
#
# def getFilenamesFromYamlDict(currLevel, saveDict=None, prevKey='', isList=False, **kwargs):
#     if saveDict is None:
#         saveDict = {}
#     if 'dir' in currLevel:
#         # print(currLevel)
#         filenames = getFilenames(currLevel)
#         # if 'usedSeqs' in kwargs:
#         #     filenames = filenames[kwargs['usedSeqs']]
#         saveDict[prevKey] = filenames
#         return (saveDict)
...
#         listVals = []
#         for i in range(len(currLevel)):
#             val = getFilenamesFromYamlDict(currLevel[i], saveDict, prevKey, isList=False, **kwargs)
#             listVals.append(val[prevKey])
#         saveDict[prevKey] = listVals
#         return (saveDict)
#
#     else:
#         if type(currLevel) is list:
#             saveDict = getFilenamesFromYamlDict(currLevel, saveDict, prevKey, isList=True, **kwargs)
#         else:
#             for key in currLevel:
#                 saveDict = getFilenamesFromYamlDict(currLevel[key], saveDict, key, isList=False, **kwargs)
#         return saveDict
#
#
# def fillInThesisReference(dictionary, thesisDict):
#     for key in dictionary:
#         dir = dictionary[key]['dir']
#         for idx, name in enumerate(dir):
#             phrase = 'thesis.'
...
#
#
# def loadConfig(expCfgName=None):
#     if expCfgName=='--mode=server' or expCfgName is None:
#         expCfgName = 'config/experiment.yaml'
#
#     expCfg = loadYamls(expCfgName)
#     if expCfg:
#         print('Loaded Config from %s' % expCfgName)
#
#     thesisCfg = loadYamls(expCfg['experiment']['thesisCfg'])
#
#     if 'kitti' in thesisCfg['datasets']:
#         kitti = thesisCfg['datasets']['kitti']
#         kitti['paths']['original'] = getFilenamesFromYamlDict(kitti['paths']['original'])
#         kitti['paths']['prepared'] = getFilenamesFromYamlDict(kitti['paths']['prepared'])
#
#     expCfg['thesisCfg'] = thesisCfg
#
#     trainPaths = expCfg['trainPaths']
#     trainPaths = fillInThesisReference(trainPaths, thesisCfg)
#     trainPaths = getFilenamesFromYamlDict(trainPaths)
#     expCfg['trainPaths'] = trainPaths
#
#     return expCfg
