...
for test in tests:
    ...
    files = sorted(glob.glob(evalFolder + '/%s*%s*,*,*%s' % (evalType, 'bias', 'hdf5')))

    axisDict = {'bias': np.empty(0), 'means': np.empty((0,numOutputs)), 'stds': np.empty((0, numOutputs))}
    axisNames = ['x', 'y', 'z']
    dataDict = {}
    ...
    for file in files:
        with h5py.File(file, 'r') as f:
            ...
            imu_bias_dph = np.array(f['imu_bias_dph'])
        ...
        axisNums = np.nonzero(imu_bias_dph)[1]
        if axisNums:
            axisNums = range(len(axisNames))

        for axisNum in axisNums:
            axis = axisNames[axisNum]
            dataDict[axis]['bias']  = np.append(dataDict[axis]['bias'], [imu_bias_dph[0,axisNum]], axis=0)
    ...
    for axis in dataDict:
        sortOrder = np.argsort(dataDict[axis]['bias'])
    ...
    for axisNum, axis in enumerate(axisNames):
        bias = dataDict[axis]['bias']
        means = dataDict[axis]['means']
        stds = dataDict[axis]['stds']

        minIdxs = np.argwhere(bias >= biasMinMax[0])
        maxIdxs = np.argwhere(bias <= biasMinMax[1])
        firstIdx = 0
        if len(minIdxs)>0:
        ...
        for outputNum in range(numOutputs):
            # mean plot
            ax1.semilogx(bias[firstIdx:lastIdx+1], means[firstIdx:lastIdx+1,outputNum],
                         colors[axisNum]+style[outputNum], label=axis+'-'+labels[outputNum], marker=marker[outputNum])

            # std dev plot
            ax2.semilogx(bias[firstIdx:lastIdx+1], stds[firstIdx:lastIdx+1,outputNum],
                         colors[axisNum]+style[outputNum], label=axis+'-'+labels[outputNum], marker=marker[outputNum])
    ...
    # ax1.legend()
    ax1.set_title('%s - Mean and Std Dev of Errors vs. Bias' % test)

    ax2.set_xlabel('Bias (deg/hr)')
