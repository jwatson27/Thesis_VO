...
def buildFlowNet_half(image_shape):   # Should use:  64*19 x 64*5 == (1216x320)
    cnn_input = Input(shape=image_shape) # (1280 x 384 x 6)
    cnn_internal = cnn_input
    ...
    cnn_output = Dense(64, activation='relu')(cnn_internal)
    cnn_model = Model(inputs=[cnn_input], outputs=[cnn_output], name='FlowNet_half')
    return cnn_model

def buildFlowNet_quarter(image_shape):   # Should use:  64*19 x 64*5 == (1216x320)
    cnn_input = Input(shape=image_shape) # (1280 x 384 x 6)
    cnn_internal = cnn_input
    cnn_internal = Conv2D(16,   (7,7), strides=2, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(32,  (5,5), strides=2, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(64,  (5,5), strides=2, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(64,  (3,3), strides=1, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(128,  (3,3), strides=2, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(128,  (3,3), strides=1, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(128,  (3,3), strides=2, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(128,  (3,3), strides=1, padding='same', activation='relu')(cnn_internal)
    cnn_internal = Conv2D(256, (3,3), strides=2, padding='same')(cnn_internal)
    cnn_output = Dense(32, activation='relu')(cnn_internal)
    cnn_model = Model(inputs=[cnn_input], outputs=[cnn_output], name='FlowNet_quarter')
    return cnn_model
...
def getCnnModel(image_shape, cnn_type=None, dropout=None):
    ...
    elif cnn_type == 'FlowNet_quarter':
        return buildFlowNet_quarter(image_shape)
    else:
        return buildCnnModel_1(image_shape)